...
// Improved eyes function with zigzag mouth
function createEyes() {
  ...
  leftEye.position.set(-0.55, 0.45, 1.95);
  leftEye.lookAt(new THREE.Vector3(-0.55, 0.45, 5));
  ...
  rightEye.position.set(0.55, 0.45, 1.95);
  rightEye.lookAt(new THREE.Vector3(0.55, 0.45, 5));
  eyeGroup.add(rightEye);

  // Mouth (W shape with more zigzag)
  const mouthPath = new THREE.CurvePath();
  const points = [
    new THREE.Vector3(-0.7, -0.1, 1.85),
    new THREE.Vector3(-0.525, -0.6, 1.92),
    new THREE.Vector3(-0.35, -0.15, 1.98),
    new THREE.Vector3(-0.175, -0.6, 2.02),
    new THREE.Vector3(0, -0.15, 2.05),
    new THREE.Vector3(0.175, -0.6, 2.02),
    new THREE.Vector3(0.35, -0.15, 1.98),
    new THREE.Vector3(0.525, -0.6, 1.92),
    new THREE.Vector3(0.7, -0.1, 1.85)
  ...
  for (let i = 0; i < points.length - 1; i++) {
    mouthPath.add(new THREE.LineCurve3(points[i], points[i+1]));
  }

  const mouthGeometry = new THREE.TubeGeometry(mouthPath, 32, 0.08, 8, false);
...
function animate(timestamp) {
  ...
  if (Math.random() < 0.03) {
    analogDecayPass.uniforms.uAnalogIntensity.value = 1.2 + Math.random() * 1.5;
    analogDecayPass.uniforms.uAnalogJitter.value = 1.0 + Math.random() * 2.0;
    analogDecayPass.uniforms.uAnalogBleeding.value = 1.0 + Math.random() * 2.0;
  } else {
    analogDecayPass.uniforms.uAnalogIntensity.value += (0.6 - analogDecayPass.uniforms.uAnalogIntensity.value) * 0.1;
    analogDecayPass.uniforms.uAnalogJitter.value += (0.4 - analogDecayPass.uniforms.uAnalogJitter.value) * 0.1;
    analogDecayPass.uniforms.uAnalogBleeding.value += (1.0 - analogDecayPass.uniforms.uAnalogBleeding.value) * 0.1;
  }
  ...
  // Eyes are permanently visible, no dynamic opacity needed
...
if (langBtn && mainQuote) {
  const textES = "QUE HACES AQUI?!!<br/>QUIERES SER PARTE DE ESTO?<br/>....PROXIMAMENTE";
  const textEN = "WHAT ARE YOU DOING HERE?!!<br/>DO YOU WANT TO BE PART OF THIS?<br/>COMING SOON..";
  ...
  if (currentLang === 'es') {
    ...
    mainQuote.innerHTML = textES;
  } else {
    langBtn.textContent = "ES";
    mainQuote.innerHTML = textEN;
  }
